...
如果上面的部分电路两边接入恒压电源，提高 $R_3$ 会使得 $R_3$ 两端电压如何变化？

$R_3$ 上升，$R'$ 上升，$R''$ 上升，于是 $R''$ 分电源压更多，电压增大；$R'$ 电压即 $R''$ 电压，也增大；$R_3$ 在 $R'$ 电压升高的同时，分 $R'$ 的压也更多，于是 $R_3$ 两端电压升高．

对于电路动态分析问题，后面会提供更多迅捷方法．此处仅展示基本功——如何分析一个混联电路．

### 复杂形态的电路简化

复杂形态的电路，如[理想导线的基本性质](#理想导线的基本性质)中给出的例子：

![示例图](./assets/partial/3.2.png)

其简化方式就是将同一段导线染色后，分析电流的走向．这里电流从电源正极出发，先经过红色导线无分支地穿过 $R_1$，变到蓝色导线中；蓝色导线接着分支地流向 $R_2$，$R_3$ 和 $R_4$，并且最后同时变成了绿色导线，因此 $R_2$，$R_3$，$R_4$ 应为并联．最终绿色导线流向电源负极．

因此整个电路的等效电路为：

![等效电路](./assets/partial/3.3.png)

### 电路的其它连接方式

电路并不是只有串联，并联和混联．考虑下面这个电路图：

![经典电路图](./assets/partial/3.4.png)
...
![用电器被短路示意图](./assets/partial/3.5.png)
...
![两头接上不一定是并联](./assets/partial/3.6.png)

$R_2$ 与 $R_3$ 两端被同一导线接上，但并没有被短路．上面的情形中，绿色导线两头接上的部分中间还有来自电源正极的电流涌入，两边的绿导线都是出口，所以短路的推理不再成立．

部分短路的原理是：当一段导线包住的电路可以等效为用电器（即电荷没有其它出入口）时，因为电荷只能从导线的一头进，另一头出，而入口和出口是同一段导线，导致内部为等势体．

### 用电器不参与回路

还有一种情形是 **用电器不参与回路**．

![不参与回路](./assets/partial/3.7.png)

如图所示的电路中，假设 $a$，$b$ 分别接入电源的正负极，则电流只会依次流过 $R_1$，$R_5$ 和 $R_2$，不会流过 $R_3$ 和 $R_4$，因为这两个电阻不参与回路．
