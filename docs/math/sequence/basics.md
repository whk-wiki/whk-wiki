...
根据二次函数的性质可得，$a$ 的最大值在对称轴 $n = \dfrac{29}4$ 取到．
...
- **作商法**：
	- 如果 $a$ **恒正**，$\dfrac{a_{n + 1}}{a_n} > 1$ 说明数列递增，$< 1$ 说明递减．
	- 如果 $a$ **恒负**，$\dfrac{a_{n + 1}}{a_n} < 1$ 说明数列递增，$> 1$ 说明递减．
...
已知数列 $a$ 满足 $a_1 = 2$，$2a_na_{n + 1} = {a_n}^2 + 1$，求证：$\left\{\dfrac{a_n - 1}{a_n + 1}\right\}$ 递减．

:::

:::tip 例题 3.4 解答

设待证数列为 $b_n = \dfrac{a_n - 1}{a_n + 1}$．分离常数得 $b_n = 1 - \dfrac 2 {a_n + 1}$．
...
$$
a_{n + 1} - a_n = \dfrac{{a_n}^2 + 1}{2a_n} - a_n = \dfrac{1 - {a_n}^2}{2a_n}
$$

已知 $a_n > 0$，分母为 $0$，只需 **考察分子的正负，即 $a_n$ 与 $1$ 的大小关系**．

注意到 $a_{n + 1} = \dfrac{{a_n}^2 + 1}{2a_n} = \dfrac{a_n} 2 + \dfrac 1 {2a_n} \ge 2 \sqrt{\dfrac{a_n} 2 \cdot \dfrac 1 {2a_n}} = 1$，当且仅当 $\dfrac{a_n}{2} = \dfrac 1 {2a_n}$，即 $a_n = 1$ 时，等号成立．

而 $a_1 = 2$，不满足等号成立条件，因此 $a_2 > 1$．因为 $a_2 \ne 1$，$a_3 > 1$……综上，**整个 $a$ 始终大于 $1$**．

因此 $1 - {a_n}^2 < 0$，$a_{n + 1} - a_n < 0$．$a$ 递减．

$\{a_n + 1\}$ **恒正递减**．于是，$b_n = 1 - \dfrac 2 {a_n + 1}$ 递减，证毕．
