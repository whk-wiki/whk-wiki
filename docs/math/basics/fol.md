...
逻辑学是一门很有深度也很有难度的学科，深入而系统地理解它不是高中数学所能承担的任务．本文讲解的内容只是逻辑学中的一点皮毛，但在高中数学的学习中应已足够．
...
变量虽然不是数字，但其具有非同寻常的意义，如乘法分配律无法只用常数表达，却可以用 $(a + b) \times c$ 方便地表达；甚至加法本身都很难用常数去表达，却能用 $a + b$ 方便地表达．引入命题变元也是为了更好地表达运算与运算律，接下来我们看命题之间的运算．
...
命题公式举例：$P$、$\neg P \land Q$、$(P \land T) \lor Q$．

命题公式不一定是命题，单个命题常元或命题变元一定是命题公式．
...
- $A \iff B$ 本身也具有真假性，因此本身可以视作命题（不严谨地来讲，严谨的逻辑学似乎规避这种操作）．
...
这四个重言蕴含关系看起来都毫无道理，但前三个均是正确的，只有最后一个是错误的．事实上，假命题可以推出任一命题，任一命题也能推出任一真命题，即使他们毫无关系．
...
我们可以用一个字符串表示谓词，比如将「是实数」表达为 $\mathrm{isReal}(x)$．因为它的自变量只有一个 $x$，称作 **一元谓词**．同时还有 **多元谓词**，$a > b$ 的谓词 $\mathrm{isBiggerThan}(a, b)$ 就是一个二元谓词．**命题** 可以视作 **零元谓词**．当然，我们还可以将 $x \in \R$ 和 $a > b$ 这类表达式直接当成谓词使用，不必非得写为函数形式，此时可能会在表达式两侧括中括号 $[]$，这个括号还有个名字「艾弗森括号」．
