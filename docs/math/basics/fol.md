...
逻辑学是一门很有深度也很有难度的学科，本文讲解的内容只是逻辑学中的一点皮毛，但已经足够应付高中数学并让读者对高中数学的逻辑有深入的理解．
...
恒等律 $P \land P \iff P$，$P \lor P \iff P$、同一律 $P \land \mathrm T \iff P$、零律 $P \land \mathrm F \iff \mathrm F$，补余律 $P \land \neg P \iff \mathrm F$、双重否定律 $P \iff \neg \neg P$．
...
量词可以手动限制论域，形式为 $\forall / \exist S(x), P(x)$，表示 $x$ 的论域为 $S(x)$ 成立时的所有 $x$ 构成的集合．如 $\forall x > 1, P(x)$，意为 $x$ 的论域变为 $[1, +\infty)$．
...
- $\forall xP(x)$ 和 $\exists xP(x)$ 中，$x$ 称作 **约束变元**．
...
所有正方形都是长方形：$\forall x \text{ 是正方形}, x \text{ 是长方形}$．或者用函数表达，$\forall \mathrm{isSquare}(x), \mathrm{isRect}(x)$．

有些平行四边形是矩形：$\exists x \text{ 是平行四边形}, x \text{ 是矩形}$．函数表达：$\exists \mathrm{isPara}(x), \mathrm{isRect}(x)$．

### 论域转限制

如果我们强制量词不限制论域，全局论域为全体几何图形，上面的命题该如何表述?
...
这里 $P(x) \to Q(x)$ 应当理解为「$x$ 可以不满足 $P$，但 $x$ 一旦满足 $P$ 就得满足 $Q$」．如果我们将 $P(x) \to Q(x)$ 中的 $P(x)$ 为假时整体定义为假，那么 $P(x) \to Q(x)$ 就连 $x$ 不满足 $P$ 的机会就不给了，我们就很难写出上面那样简洁的式子．$\to$ 这个联结词其实主要是为了方便 $\forall$．
...
有些平行四边形是矩形，等价于有几何图形同时满足平行四边形和矩形两个命题．

其实在这个情境下，我们需要的就是一个类似 $P \to Q$，但 $P$ 为假时恒假的「蕴含」运算符，但仔细思考一下就会发现，这样的运算符其实就是 $\land$，因此 $\land$ 已经足够．
...
$$
\begin{aligned}
\forall S(x), P(x) &\iff \forall x, S(x) \to P(x) \\
\exist S(x), P(x) &\iff \exist x, S(x) \land P(x)
\end{aligned}
$$

本文称前者为「**论域表达**」，后者为「**规范表达**」（因为逻辑学中更常用的写法是后者）．

高中阶段更偏好「论域表达」，「规范表达」不常见（因为高中阶段甚至未引入命题联结词）．**请读者尽量使用「论域表达」**．然而本文默认会使用「规范表达」形式，因为「规范表达」直观表现了命题函数的逻辑学本质．
...
$A$ 是 $B$ 的充分不必要条件等价于 $A$ 比 $B$ 强；$A$ 是 $B$ 的必要不充分条件等价于 $A$ 比 $B$ 弱．可以看到，在一个推出链 $A \implies B \implies C$ 上，$A$，$B$，$C$ **单调不变强**（即保持强度相同或变弱）．举个例子：$x > 3 \implies x > 2 \implies x > 1$．
...
相当于 $P(x, y) \to Q(x)$ 在任意 $x$，$y$ 的指派下均为恒真式，即

$$
P(x, y) \implies Q(x)
$$

此时，称 $P(x, y)$ 为 $Q(x)$ 的充分条件，$Q(x)$ 为 $P(x, y)$ 的必要条件．
