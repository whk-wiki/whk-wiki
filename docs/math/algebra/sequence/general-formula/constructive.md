...
在[基础求通项](./common.md)问题中，我们了解到一类 **有提示构造** 的通项处理思路：对于未知通项求法的数列 $a$，我们可以找到一个与 $a$ 有简单关系的数列 $b$，从而让 $b$ 的通项可知，进而求解 $a$ 的通项．
...
因此 $b$ 为首项 $-\df 2 3$，公比 $\df 2 3$ 的等比数列，有 $b_n - 1 = -\df 2 3 \cdot (\df 2 3)^{n - 1} = -\df{2^n}{3^n}$．
...
- 求 $f$ 的不动点，即 $x = \df{px + q}{sx + t} \iff sx^2 + (t - p)x - q = 0$ 的解，为二次方程．
	...
	- 若 $f$ 恰存在一个不动点 $x_0$，换元 $b_n = \* \df 1 {a_n - x_0}$，$b$ 会是一个等差数列．
	- 若 $f$ 存在两个不动点 $x_1$，$x_2$，换元 $b_n = \* \df{a_n - x_1}{a_n - x_2}$，$b$ 会是一个等比数列．
