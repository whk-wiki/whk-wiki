...
值得注意的是，这种做法 **规避了分类讨论**，小细节是这里的 $g$ 不要写成数列，否则 $g_0$ 就无定义了，又要分类讨论．
...
$$
a_{n + 1} - a_n = n + 1 = (\df 1 2 n^2 + \df 3 2 n) - [\df 1 2 (n - 1)^2 + \df 3 2 (n - 1)]
$$

因此有

$$
a_{n + 1} - (\df 1 2 n^2 + \df 3 2 n) = a_n - [\df 1 2 (n - 1)^2 + \df 3 2 (n - 1)]
$$

即 $\{a_n - [\df 1 2 (n - 1)^2 + \df 3 2 (n - 1)]\}$ 是一个常数列，$a_n - [\df 1 2 (n - 1)^2 + \df 3 2 (n - 1)] = a_1 - 0 = 2$．

因此 $a_n = 2 + [\df 1 2 (n - 1)^2 + \df 3 2 (n - 1)] = \df{n^2 + n + 2} 2$．
