...
有关 **奇偶项数列的求和** 以及很重要的 **奇偶项变换**，请读者回顾[奇偶项求和](../sum/basics.md#奇偶项求和)，这里不再重复介绍．
...
如果题目只对 $\{a_n\}$ 的前缀和 $\{S_n\}$ 进行询问，那么我们没必要求出 $\{a_n\}$ 的通项，**$\{a_n\}$ 的奇项数列与偶项数列的通项足以解决问题**．

假设我们求出了 $\{a_n\}$ 的奇项数列通项 $a_{2n - 1} = f(n)$，偶项数列通项 $a_{2n} = g(n)$．

则 $S_{2n} = f(1) + g(1) + f(2) + g(2) + \cdots + f(n) + g(n)$：

- **分开求**：拆成 $[f(1) + f(2) + \cdots + f(n)] + [g(1) + g(2) + \cdots + g(n)]$．
- **并项求**：拆成 $[f(1) + g(1)] + [f(2) + g(2)] + \cdots + [f(n) + g(n)]$．

如果题目问 $S_{2n - 1}$，先求 $S_{2n}$，则 $S_{2n - 1} = S_{2n} - g(n)$．

如果题目问 $S_n$，先求 $S_{2n}$，再求 $S_{2n - 1}$，最后利用 **奇偶项变换** 拼出 $S_n$．

#### 第二步（若询问 $a_n$）

直接将求得的 $a_{2n - 1}$ 与 $a_{2n}$ **奇偶项变换** 即得．
...
这里换了一种思路求通项，可以看到这个流程会比例题 5.1 中的思路还要简单一些．

注意上面 $n$ 为奇数时，凭借对象应该是 $n + 1$ 为偶数，而不是 $n - 1$．否则 $n = 1$ 的时候 $a_{n - 1}$ 不存在，还要特别讨论．
