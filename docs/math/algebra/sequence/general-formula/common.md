...
如果题目给出的 $\{a_n\}$ 的递推公式 **形如** 或 **可以变形为**：$a_n - a_{n - 1} = f(n)$，相当于 **给出了 $\{a_n\}$ 的差分 $\{D_n\}$ 的通项**．如果 $f(n)$ 可以求前缀和（**等差数列**、**等比数列**、**等差乘等比数列**、**裂项**……），对 $f(n)$ 做前缀和即可得到 $\{a_n\}$ 的通项，这个方法称作 **累加法**．

同理，如果 $\{a_n\}$ 递推公式形如或可变形为 $\df{a_n}{a_{n - 1}} = f(n)$，相当于给出 **$\{a_n\}$ 的「比分」$\{R_n\}$ 的通项**．如果 $f(n)$ 可以求前缀积（**等比数列**、**可约分多项的**（见例题 3.2）），对 $f(n)$ 做前缀积可得 $\{a_n\}$ 的通项，称作 **累乘法**．
...
这个题的关键是读懂出题人的意图：出题人绝对不是让求出 $x$ 的通项，然后暴力代入解出 $\{a_n\}$．出题人真正的意图是 **提示要求 $x$ 的通项，构造方向是 $\{a_n\}$**，即 $x$ 的通项可能难求，但 $\{a_n\}$ 的通项会比较好求．
...
题目并没有问 $x$ 的通项是什么，但仍然能看出：$\{a_n\}$ 就是要求 $x$ 的通项的一个良好构造．
...
因此 $\{a_n\}$ 的奇数项，偶数项分别构成公差为 $2$ 的等差数列，奇数项首项为 $a_1 = 1$，偶数项首项为 $a_2 = (a_1 + a_2) - a_1 = 2 - 1 = 1$．
...
接下来的题目中，$\{S_n\}$ 的通项均是未知的，而只给出 $\{S_n\}$ 与 $\{a_n\}$ 的一个关系式．这类题有两个比较固定的解题策略：**消 $\{S_n\}$** 和 **消 $\{a_n\}$**．先来看 **消 $\{S_n\}$**．
...
在 $n \ge 2$ 时，上面的例题中给出的所有式子，都能通过 **再向前递推一项**，然后 **作差** 将 $\{S_n\}$ 直接消去．然而并不是所有的式子的 $\{S_n\}$ 都如此好消，如
...
此时就要换一种策略：**消 $\{a_n\}$**．考虑到 $n \ge 2$ 时，$a_n = S_n - S_{n - 1}$，有
