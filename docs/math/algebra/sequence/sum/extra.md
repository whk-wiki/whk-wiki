...
- **猜出来的形式一定要是一个合法裂项**，即分母可以统一成一个数列 $b$ 的第 $n$ 项与第 $n + k$ 项，分子也能统一成一个数列 $c$ 的第 $n$ 项与第 $n + k$ 项．两个 $k$ 是相同的．
...
这种形式的裂项无法用来直接求 $a$ 的前缀和，但是可以求 $\{(-1)^n a_n\}$ 的前缀和，见下例：

:::info 例题 1.7.1

已知 $a_n = (-1)^n \df{4n}{4n^2 - 1}$ 前缀和为 $S$，求 $S_{2020}$．
...
类比等差数列 $a$ 的前缀和 $S$ 用倒序相加可以证明

$$
2S_n = n(a_1 + a_n)
$$

等比数列 $b$ 的前缀积 $T$ 用倒序相乘可以证明
