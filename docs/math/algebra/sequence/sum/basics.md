...
- 字面意思地给出：已知等比数列 $a$，等差数列 $b$，则 $\{a_n \cdot b_n\}$ 为等差乘等比数列．
...
因此可以检查自己的计算结果 $S$ 是否满足：

- 符合结构 $\{(Xn + Y) \cdot q^n - Y\}$．
- 满足 $S_1 = a_1$．

如果两个条件有一个不满足说明 $S$ 一定算错了，如果都满足说明很大概率算对．
...
- $p_n$ 与 $q_n$ 可以刻画为 **同一个数列** $b$ 的第 $n$ 项与第 $n + k$ 项，$k$ 是一个常数．
...
设 $\{\df 1 {b_n}\}$ 的前缀和为 $T$，则考察对象为 $T_n - (T_{n + k} - T_k) = T_n + T_k - T_{n + k}$．从这个式子可以看出，**考察对象的 $n$ 与 $k$ 互换，结果不变**．
...
这一步笔者给出的理解是：$\{\df 1 n\}$ 的前缀和 $T$，有 $T_n - (T_{n + 2} - T_2) = T_2 - (T_{n + 2} - T_n)$，将两段长度为 $n$ 的和相减转化为长度为 $2$ 的和相减．
...
- $p_n$ 与 $q_n$ 可以刻画为 **同一个数列** $b$ 的第 $n$ 项与第 $n + k$ 项，$k$ 是一个常数．这个条件是为了 **最后裂成的分式可以统一为某个数列的一个连续段减去另一个连续段的形式**．
...
已知 $a$ 的通项公式为 $a_n = 3 \cdot 4 ^{n - 1}$，$a$ 的前缀和为 $S_n$，$b_n = \df{a_{n + 1}}{(a_{n + 1} - 3)S_{n + 1}}$，求 $b$ 的前缀和 $T_n$．
...
已知 $b_n = n \cdot 2^{n - 1}$，$a_n = \df{b_{n + 2}}{b_nb_{n + 1}}$，求 $a$ 的裂项．

:::

:::tip 例题 2.3.5 解答

$a$ 的通项分母已经自动表示成一个数列的第 $n$ 项和第 $n + 1$ 项了，此时第一反应一定是算一下 $\df{分子}{分母差}$：
...
已知递增等差数列 $a$ 前三项和为 $-9$，前三项积为 $-15$，求 $\{\lv a_n \rv\}$ 的前缀和 $S_n$．
...
内容很简单：如果 $a$ 的前缀和为 $S$，$b$ 的前缀和为 $T$，则 $\{a_n + b_n\}$ 的前缀和为 $\{S_n + T_n\}$．
...
小细节：常数 $c$ 构成的常数列求和是 $nc$，这个 $n$ 比较容易遗漏．如 $3^{n - 1} + 1$ 求和的结果应为 $\df{3^n - 1} 2 + \* n$．
...
由于 $a_{n + 1} - a_n = 2 \times 3^{n - 1} - 1 > 0$，因此 $a$ 递增，当 $n \ge 3$ 时，$a_n \ge a_3 > 0$．
...
给定一个数列 $a$ 和一个数列 $b$，求 $c_n = \bcs a_n, & n 为奇数 \\ b_n, & n 为偶数 \ecs$ 的前缀和 $S_n$．

$$
S_n = a_1 + b_2 + a_3 + b_4 + \cdots
$$

这类题目的 $S$ 通常也是分段写的（奇偶通项不同）．
...
需要注意：第一部分并不是 $a$ 的前缀和，求和形态也与 $a$ 数列有差异．如 $a$ 若是一个公差为 $2$ 的等差数列，则第一部分中的求和中公差为 $4$．

:::info 例题 5.1

已知 $c_n = \bcs 2n - 1, & n 为奇数 \\ 3^{n - 1}, & n 为偶数 \ecs$ 的前缀和 $S$，求 $S_{2n}$．
...
- $S_{2n} = p_n$．
- $S_{2n - 1} = p_n - b_{2n}$．
- $n$ 为偶数时，$S_n = p_{\frac n 2}$．
- $n$ 为奇数时，$S_n = p_{\frac {n + 1} 2 } - b_{n + 1}$．
...
上面的求和使用等差、等比求和都十分容易，为什么要专门拉出来作为结论呢？因为 **出现频率实在太高了**，遇到这些求和的时候直接用结论写答案可以提升解题效率．
