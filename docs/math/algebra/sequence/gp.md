...
- 当 $q < 0$ 时，$a$ 为 **摆动数列**（正负摆动，因此增减摆动）．如果想要探讨 $\{|a_n|\}$ 的增减性，将 $q$ 与 $a_1$ 取绝对值再分析即可．
- 当 $q = 1$ 时，$a$ 为 **常数列**．
...
- $q < 0$ 时，$a$ 的 **正负摆动**，**增减性摆动**，通常不讨论．
- $q > 0$ 时，$a$ **恒正 / 恒负**．
- $q > 0$ 时，如果 $q$ 足够 **贴近** $0$（$0 < q < 1$），则数列 $a$ 也会 **贴近** $0$（表现为 **绝对值递减**）；如果 $q$ 距离 $0$ **比较远**（$q > 1$），则数列 $a$ 会 **远离** $0$（表现为 **绝对值递增**）．
...
显然 $a_1 < 0$ 且 $q < 0$ 满足「$a_1(q - 1) > 0$」，但 $a$ 摆动，而不满足「$a$ 递增」．
...
对于 **正项** 等比数列 $a$（正项自动提出 $q > 0$ 的要求），令 $b_n = \lg a_n$，则：

- $b_n = \lg(a_1 \cdot q^{n - 1}) = \lg a_1 + (n - 1) \lg q$，即 $b$ 是一个 **首项** 为 $\* \lg a_1$，**公差** 为 $\* \lg q$ 的 **等差数列**．
- 【$\*a_n$ 与 $\* 1$ 的大小关系】和【$\*b_n$ 与 $\* 0$ 的大小关系】**保持同步**．
- 令 $a$ 的 **前缀积** 为 $T$，$b$ 的 **前缀和** 为 $S$，有 $\* S_n = \lg T_n$．
- 【$\*T_n$ 与 $\* 1$ 的大小关系】和【$\*S_n$ 与 $\* 0$ 的大小关系】**保持同步**．

:::info 例题 2.5

已知正项等比数列 $a$ 满足 $a_1 > 1$，$a_6 + a_7 > a_5a_8 + 1 > 2$，记 $T$ 为 $a$ 的前缀积，下列选项中正确的是（多选）：
...
已知等差数列 $b$ 满足 $b_1 > 0$，$b_6b_7 < 0$，$b_6 + b_7 > 0$，记 $S$ 为 $b$ 的前缀和，下列选项中正确的是（多选）：
...
- A. $b$ 下降，$d < 0$，正确．
- B. $b_6 > 0$，$b_7 < 0$，正确．
- C、D. $b$ 的零点位于 $(6.5, 7)$，因此 $S$ 的对称轴横坐标位于 $(6, 6.5)$，可知 $S$ 的零点位于 $(12, 13)$．且 $S$ 开口向下，因此 $S_{12} > 0$，$S_{13} < 0$，C 正确，D 错误．
...
对于不保证正项的等比数列，我们有 $\{\lg \lv a_n \rv\}$ 为公差 $\{\lg \lv q \rv\}$，但是这个形式就不那么好了，一般不讨论．

等差其实也可以转等比，如果 $a$ 为公差为 $d$ 的等差数列，则 $\{t^{a_n}\}$（$t \ne 0$）是公比为 $t^d$ 的等比数列．
