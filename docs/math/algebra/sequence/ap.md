...
- **等差数列前缀和的通项公式一定符合形态 $An^2 + Bn$**．是一个 **最高不超过二次** 的，**常数项为零** 的函数．
- 因为函数 $S$ **常数项为零**，因此 $\*S(0) = 0$．$S$ 的图像 **过原点**．
- $\* d \ne 0$ 时 $S$ **一定是二次函数**．$\*d > 0$ 时，函数 **开口向上**；$\*d < 0$ 时，函数 **开口向下**．
- $S(n) = An^2 + Bn = n(An + B)$，天然 **可以因式分解**，很有助于做题．

同时，在 $An^2 + Bn$ 中，对于任意 $A, B \in \R$，方程组 $\bcs A = \fr d 2 \\ B = a_1 - \fr d 2 \ecs$ 恒有解 $\bcs a_1 = A + B \\ d = 2A\ecs$，

因此通项公式符合形态 $An^2 + Bn$ 的（**注意不能有常数项**）的数列一定是 **某个等差数列的前缀和**，或者说 **其差分一定是个等差数列**．
...
当 $d \ne 0$ 时，函数 $S$ 为二次函数，设函数 $S$ 的 **对称轴横坐标** 为 $n_S$，函数 $a$ 的 **零点** 为 $n_A$，我们有
...
可以发现，$\* n_S = n_A - \df 1 2$，即 $\{S_n\}$ 的对称轴恰好相对 $\{a_n\}$ 的零点 **左偏** $\* \df 1 2$，该性质 **非常有助于 $\{S_n\}$ 与 $\{a_n\}$ 的形态互推**．
...
首先，$S(n)$ 一定为二次函数（过原点的一次函数或常零函数不可能满足 $S(2023) < 0$ 且 $S(2024) > 0$）．
