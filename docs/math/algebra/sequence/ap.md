...
$\left\{\dfrac{S_n}n\right\}$ 是一个首项为 $a_1$，公差为 $\* \dfrac 1 2 d$（$d$ 为 $a$ 的公差）的 **等差数列**．反过来，$\left\{\dfrac{S_n}n\right\}$ 是等差数列也可推出 $a$ 是等差数列．
...
- $S$ 的对称轴横坐标 $n_S = -\dfrac{B}{2A} = \dfrac 1 2 - \dfrac{a_1}d$，$a$ 的零点 $a_1 + (n_A - 1)d = 0$，解得 $n_A = 1 - \dfrac{a_1}d$．因此，$\* n_S = n_A - \dfrac 1 2$，即 **$S$ 的对称轴恰好相对 $a$ 的零点左偏 $\* \dfrac 1 2$**，该性质 **非常有助于 $S$ 与 $a$ 的形态互推**．
...
可看出 $a$ 的零点位于 $\left(12.25, 13\right)$，且单调递减，$d < 0$．
