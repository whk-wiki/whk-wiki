...
这里 $n$ 可以是任意整数，而导出的 $S$ 的下标一定是奇数．

#### 等差数列性质五

$\{\df{S_n}n\}$ 是一个首项为 $a_1$，公差为 $\* \df 1 2 d$（$d$ 为 $a$ 的公差）的 **等差数列**．反过来，$\{\df{S_n}n\}$ 是等差数列也可推出 $a$ 是等差数列．
...
在等差数列 $a$ 中，$a_9 = \df 1 2 a_{12} + 3$，求 $S_{11}$．
...
已知两个等差数列 $a$，$b$，前缀和分别为 $S$，$T$，已知 $\df{S_n}{T_n} = \df{7n + 2}{n + 3}$，求 $\df{a_7}{b_7}$ 的值．
...
设 $S$ 是等差数列 $a$ 的前缀和，若 $\df{S_3}{S_6} = \df 1 3$，求 $\df{S_6}{S_{12}}$ 的值．
...
已知数列 $a$ 满足 $a_1 = 4$，$a_{n + 1} = \df{a_n}{a_n + 1}$，求证：$\{\df 1 {a_n} \}$ 是等差数列．
...
已知数列 $a$ 满足 $a_1 = 1$，$n(a_{n + 1} - a_n) = a_n + n^2 + n, \quad n \in \N^\ast$，求证：$\{\df {a_n} n \}$ 是等差数列．
...
已知等差数列 $a$，前缀和 $S$，$a_1 = 9$，$\df {S_9} 9 - \df {S_5} 5 = -4$，求 $S$ 的最大项．
...
- 等差数列前缀和的通项公式一定是 **无常数项的二次函数**（$d \ne 0$）或 **无常数项的一次函数**（$d = 0$），因此，$S$ 在图像上表示为 **过原点的** 一 / 二次函数，即 $S(0) = 0$．在画图时要注意过原点．
...
这个函数形式的基本用法是：因为二次项 $\df d 2$ 和一次项 $a_1 - \df d 2$ 都很好记，因此当要求某个等差数列前缀和 $S$ 的通项公式时：
...
设 $S$ 的 **对称轴横坐标** 为 $n_S$，$a$ 的 **零点** 为 $n_A$，我们有
...
可以发现，$\* n_S = n_A - \df 1 2$，即 **$S$ 的对称轴恰好相对 $a$ 的零点左偏 $\* \df 1 2$**，该性质 **非常有助于 $S$ 与 $a$ 的形态互推**．
...
已知等差数列 $a$ 前缀和 $S$，若 $S_{2023} < 0$，$S_{2024} > 0$，下列结论正确的是（  ）【多选】．
...
- A. $a$ 递增．显然正确．
- B. $|a_{1013}| < |a_{1012}|$．错误，$1012$ 距离 $a$ 的零点更近，绝对值更小．
- C. $S_{1012}$ 为 $S$ 的最小项．正确，$1012$ 是距离 $S$ 对称轴最近的整点．
- D. $S_{1015} > S_{1008}$．对称轴距离 $1015$ 的距离范围 $(3, 3.5)$，距离 $1008$ 的距离范围 $(3.5, 4)$，$1015$ 距离对称轴更近，函数值更小，错误．

因此选择 AC．

可以看出，这种题只要根据条件将 $a$ 与 $S$ 的图像绘制正确，难度很低．
...
**下标守恒定律中，所有下标推广到实数并变成函数自变量，定律仍然成立**．因为当时推下标守恒定律的时候，只用到了 $a$ 的通项，并没有用到 $a$ 的离散性．
...
已知 $S$ 为等差数列 $a$ 的前缀和，$S_{15} < S_{10}$，且 $a_1 + a_{14} + a_{15} + a_{19} > 0$，求 $S$ 的最大项．
...
可看出 $a$ 的零点位于 $(12.25, 13)$，且单调递减，$d < 0$．

因此，$S$ 开口向下，对称轴位于 $(11.75, 12.5)$，可看出 $12$ 一定是与对称轴最近的整点，因此答案为 $S_{12}$．
...
这一点可能需要重新证明，直接代入 $S$ 与 $a$ 的解析式即可得证．
